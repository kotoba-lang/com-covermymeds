(ns covermymeds.main
  "Kotodama WASM entrypoint for the Covermymeds clean-room actor (L4) — Clojure port.
  ...
  facts (`covermymeds.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "covermymeds")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "covermym_pre"
   ...
   {:entity "Claim"       :plural "claims"       :id-prefix "covermym_cla"
   ...
   {:entity "Pharmacy"    :plural "pharmacies"   :id-prefix "covermym_pha"
    :fields [:npi :name :network]                :required [:npi :name]
    :coerce {}                                :refs {}}
   {:entity "Drug"       :plural "drugs"       :id-prefix "covermym_dru"
    :fields [:ndc :name :strength :form]         :required [:ndc :name]
    :coerce {}                                :refs {}}
   {:entity "Member"     :plural "members"     :id-prefix "covermym_mem"
    :fields [:memberId :planId :groupId]         :required []
    :coerce {}                                :refs {:memberId "Member"}}
   {:entity "PriorAuth"  :plural "priorauths"  :id-prefix "covermym_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"covermymeds.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "covermymeds-compat" :tier tier :entities entities} 200])
...
(defn start! [] :covermymeds-compat/ready)
